...
{
    // valida horário
    if (request.getEndHour() <= request.getStartHour()) return false;
    if (request.getStartHour() < 7 || request.getEndHour() > 21) return false;
    ...
    for (int i = 0; i < room_count; i++)
    {
        ...
        if (room_capacities[i] >= request.getStudentCount())
        {
            ...
            if (!has_conflict)
            {
                // adiciona a reserva na próxima posição livre
...
// Localiza a reserva pelo nome da disciplina e a remove
...
// Converte o dia da semana para número para permitir ordenação
int show_day(std::string dia)
{
    if (dia == "segunda") return 0;
    if (dia == "terca")   return 1;
...
// Ordena as reservas de uma sala por dia da semana e horário de início (insertion sort)
void insertionSortReservations(Reservation *reservations, int n)
{
    for (int i = 1; i < n; i++)
    {
